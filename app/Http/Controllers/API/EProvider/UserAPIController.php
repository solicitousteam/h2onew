...
class UserAPIController extends Controller
{
    ...
    function login(Request $request)
    {
        try {
            ...
            if (auth()->attempt(['email' => $request->input('email'), 'password' => $request->input('password')])) {
                ...
                $user->save();
                $provider=DB::table('e_provider_users')->where('user_id',$user->id)->first();
                if($provider){
                    $user->e_provider_id = $provider->e_provider_id;
                }else{
                    $user->e_provider_id = null;
                }
    ...
    function register(Request $request)
    {
        try {
            ...
            $user->save();
            $user_id=$user->id;
            $json_name=json_encode(array('en'=>$request->input('name')));
            DB::table('e_providers')->insert(
                 array(
                        'name'     =>   $json_name, 
                        'e_provider_type_id'=>'4',
                        'available'   =>   '1',
                        'featured'=>'1',
                        'accepted'=>'1'
                 )
            ...
            DB::table('e_provider_users')->insert(
                 array(
                        'user_id'     =>   $user_id, 
                        'e_provider_id'   => $provider_id
                 )
            );
